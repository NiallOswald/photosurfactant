...
plot_first_order --func "np.cos(np.pi * x / L) + np.cos(2 * np.pi * x / L) / 2 + np.cos(3 * np.pi * x / L) / 3" --problem inverse --wave_count 3 -s --path "./figures/inverse/" --grid_size 1000 --usetex --format "pdf"
